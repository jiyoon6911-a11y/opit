...
import { Play, Download, Trash2, RefreshCw, ChevronLeft, Settings2, Share2, BookHeart, Sparkles } from 'lucide-react';
...
export default function App() {
  ...
  if (!isLoaded) return <div className="min-h-screen bg-pastel-blue flex items-center justify-center"><div className="w-8 h-8 border-4 border-coral border-t-transparent rounded-full animate-spin" /></div>;

  if (view === 'home') {
    return (
      <div className="min-h-screen bg-pastel-blue text-ink flex flex-col items-center justify-center p-6 relative overflow-hidden">
        <div className="max-w-md w-full space-y-10 z-10">
          <div className="text-center space-y-4">
            <div className="inline-flex items-center justify-center p-4 bg-white rounded-3xl shadow-sm mb-2 rotate-[-5deg]">
              <BookHeart className="w-10 h-10 text-coral" />
            </div>
            <h1 className="text-4xl font-bold tracking-tight text-ink">나의 매치컷 기록</h1>
            <p className="text-ink/60 text-lg">
              매일매일 귀여운 조각들을 모아<br/>하나의 영상으로 이어봐요 📸
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {SHAPES.map(shape => {
              ...
              <button
                ...
                className={`relative bg-white rounded-[2rem] p-6 flex flex-col items-center gap-4 shadow-sm hover:shadow-md transition-all border-2 border-transparent ${shape.borderClass} group`}
              >
                {frameCount > 0 && (
  ...
  return (
    <div className="min-h-screen bg-pastel-blue text-ink flex flex-col">
      <canvas ref={canvasRef} className="hidden" />

      {/* Header */}
      <header className="fixed top-0 left-0 right-0 z-50 p-4 flex justify-between items-center bg-gradient-to-b from-black/50 to-transparent pointer-events-none">
        <div className="flex items-center gap-2 pointer-events-auto">
          {view === 'capture' ? (
            <button 
              onClick={handleBackToHome}
              className="px-4 py-2 rounded-full bg-white/90 hover:bg-white text-ink shadow-sm backdrop-blur-md transition-colors flex items-center gap-1 text-sm font-bold"
            >
            ...
            <div className="flex items-center gap-2 px-2 text-white">
              ...
              <h1 className="text-lg font-bold tracking-tight">매치컷 기록</h1>
            </div>
          )}
        </div>
        ...
        {view === 'capture' && (
          <button 
            ...
            className="p-3 rounded-full bg-white/90 hover:bg-white text-ink shadow-sm backdrop-blur-md transition-colors pointer-events-auto"
          >
      ...
      <main className="flex-1 w-full relative overflow-hidden flex flex-col">
        {view === 'capture' ? (
          <>
            ...
            <div className="relative flex-1 bg-ink overflow-hidden flex items-center justify-center">
              <div className="relative w-full max-w-[min(100%,(100vh-200px)*9/16)] aspect-[9/16] bg-black rounded-b-[2rem] sm:rounded-[2rem] overflow-hidden shadow-xl">
                <video 
                  ref={videoRef} 
                  autoPlay 
                  playsInline 
            ...
            {/* Controls & Timeline */}
            <div className="bg-pastel-blue pb-[env(safe-area-inset-bottom)] pt-4">
              {/* Timeline */}
              <div className="h-24 flex items-center px-6 overflow-x-auto gap-4 snap-x hide-scrollbar">
                {frames.length === 0 ? (
                  <div className="w-full text-center text-ink/50 text-sm font-bold flex items-center justify-center gap-2">
                    <Sparkles className="w-4 h-4" />
                    <span>첫 번째 조각을 기록해주세요!</span>
                  </div>
                ) : (
                  frames.map((frame, idx) => (
                    <div key={idx} className={`relative group shrink-0 snap-center bg-white p-1.5 pb-4 rounded-sm shadow-sm ${idx % 2 === 0 ? 'rotate-2' : '-rotate-2'} hover:rotate-0 transition-transform`}>
                      <div className="w-14 h-20 overflow-hidden bg-ink/5">
                        <img src={frame} alt={`Frame ${idx + 1}`} className="w-full h-full object-cover" />
                      </div>
                      <div className="absolute bottom-0.5 left-0 right-0 text-center text-[10px] font-bold text-ink/40">
                        {idx + 1}
                      </div>
                      <button 
          ...
          /* Preview View */
          <div className="flex-1 flex flex-col bg-pastel-blue">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 gap-8">
              {isGenerating ? (
                <div className="flex flex-col items-center gap-6">
                  ...
                  <p className="text-coral font-bold animate-pulse tracking-tight">기록을 엮는 중... 🎬</p>
                </div>
              ) : videoUrl ? (
                <>
                  <div className="relative w-full max-w-[min(100%,(100vh-300px)*9/16)] aspect-[9/16] bg-white p-3 pb-12 rounded-sm shadow-xl rotate-1">
                    <div className="w-full h-full bg-ink/5 overflow-hidden">
                      <video 
                        src={videoUrl} 
                        controls 
                        autoPlay 
                    ...
                    <div className="absolute bottom-4 left-0 right-0 text-center font-bold text-ink/40">
                      나의 매치컷 기록 📸
                    </div>
                  </div>
